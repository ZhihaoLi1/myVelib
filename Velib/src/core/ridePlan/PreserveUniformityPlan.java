 ...
 * one free parking slot. Then if a station s0' whose distance from the starting location
 ...
 * of s0. Similarly if a station s0d (whose distance from the destination location is at most 105% 
 * of the distance of sd from the destination location) has a larger number
 * of free parking slots than sd it should be selected as the destination station in place of sd
 * @author animato
 *
 */
public class PreserveUniformityPlan implements RidePlanStrategy {
	...
	public RidePlan planRide(Point source, Point destination, User user, String bikeType, HashMap<Integer, Station> stations) throws Exception {
		...
		return new RidePlan(source, destination, sourceStation, destStation, "preserveUniformity", bikeType);
	}
